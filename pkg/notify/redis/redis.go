...
func (r *redisNotify) NotifyCompletion(ctx context.Context, jobID, reason string, err error) error {
	if cmd := r.client.Publish(ctx, pubChannel(jobID, reason), message{
		Error: err,
	}); cmd.Err() != nil {
